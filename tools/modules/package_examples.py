...
VERSION = os.environ.get("HOVEL_MODULE_VERSION", "0.1.0")


NATIVE_MODULES = [
    ("mock-survey-go", "survey", "Example Go survey module.", "examples/bin/mock-survey-go"),
    ("mock-exploit-go", "exploit", "Example Go exploit module.", "examples/bin/mock-exploit-go"),
    (
    ...
    ("mock-survey-rust", "survey", "Example Rust survey module.", "examples/bin/mock-survey-rust"),
    ("mock-exploit-rust", "exploit", "Example Rust exploit module.", "examples/bin/mock-exploit-rust"),
    (
    ...
    ("squatter", "payload_provider", "Squatter payload provider module.", "examples/bin/squatter-provider"),
]

PYTHON_MODULES = [
    ("mock-survey", "survey", "Example Python survey module.", "examples/python/mock_survey", "hovel_example_survey"),
    (
        "mock-exploit",
        "exploit",
...
def native_manifest(name: str, module_type: str, summary: str, command: str) -> str:
...
def python_manifest(name: str, module_type: str, summary: str, module: str) -> str:
    return f"""apiVersion: hovel.dev/v1alpha1
kind: ModulePackage
metadata:
  name: {name}
...
def package_native(name: str, module_type: str, summary: str, binary: str) -> Path:
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        command = Path(binary).name
        write(root / "hovel-module.yaml", native_manifest(name, module_type, summary, command))
...
def package_python(name: str, module_type: str, summary: str, source: str, module: str) -> Path:
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        copytree(ROOT / source, root)
        write(root / "hovel-module.yaml", python_manifest(name, module_type, summary, module))
...
def main() -> None:
    ...
    for archive in sorted(archives):
        name = archive.name[: -len(f"-{VERSION}.tgz")]
