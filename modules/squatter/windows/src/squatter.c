...
static SOCKET listen_on(const wchar_t *port)
{
        ...
        if (s != INVALID_SOCKET)
        {
                (void)setsockopt(s, SOL_SOCKET, SO_REUSEADDR, (const char *)&yes, (int)sizeof yes);
                if (bind(s, res->ai_addr, (int)res->ai_addrlen) != 0 || listen(s, SOMAXCONN) != 0)
                {
                        (void)closesocket(s);
                        s = INVALID_SOCKET;
                }
...
static DWORD WINAPI on_service_control(DWORD control, DWORD event_type, LPVOID event_data, LPVOID context)
{
        (void)event_type;
        (void)event_data;
        (void)context;
...
static SOCKET connect_reverse_tcp(const sq_hovel_config *config)
{
        ...
        CopyMemory(&addr.sin_addr, config->reverse_tcp_host, sizeof config->reverse_tcp_host);
...
static void add_session_or_drop(sq_session **sessions, int *session_count, sq_session *s, const wchar_t *what)
{
...
static sq_session *session_from_socket(SOCKET client, const sq_module_table *table)
{
...
static void run_named_pipe_server(const wchar_t *pipe_name, const sq_module_table *table)
{
        ...
        SQLOG_INFO(SQLOG_SUB_MUX, L"listening on named pipe %s; connect over SMB as \\\\HOST\\pipe\\...", pipe_name);
        ...
        while (!g_stop)
        {
                ...
                {
                        SQLOG_WINERR(SQLOG_SUB_MUX, ERROR, GetLastError(), L"CreateNamedPipeW failed");
                        break;
                }
...
static int run_tcp_bind_server(const wchar_t *port, const sq_module_table *table)
{
        ...
        SQLOG_INFO(SQLOG_SUB_MUX, L"listening on port %s (module: echo); Ctrl-C to stop", port);
...
static const wchar_t *configured_port(const sq_hovel_config *config, wchar_t port_buffer[16])
{
        if ((config->kind == SQ_HOVEL_TRANSPORT_REVERSE_TCP || config->kind == SQ_HOVEL_TRANSPORT_TCP_BIND) &&
            config->reverse_tcp_port != 0)
        {
                (void)wnsprintfW(port_buffer, 16, L"%u", (unsigned)config->reverse_tcp_port);
                return port_buffer;
        }
        return L"9100";
}
...
static void WINAPI service_main(DWORD argc, LPWSTR *argv)
{
        ...
        g_service_status_handle =
            RegisterServiceCtrlHandlerExW(g_service_context.service_name, on_service_control, NULL);
        if (g_service_status_handle == NULL)
        {
                return;
        }
...
static int run_as_service(wchar_t *service_name, const wchar_t *port, const sq_module_table *table)
{
...
static void parse_main_args(int argc, wchar_t **argv, const wchar_t **port, wchar_t **service_name)
{
        int i = 0;
