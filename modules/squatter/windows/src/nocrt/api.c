...
static int ascii_has_dll_suffix(const char *s)
{
        ...
        return ascii_lower((int)s[n - 4]) == '.' && ascii_lower((int)s[n - 3]) == 'd' &&
               ascii_lower((int)s[n - 2]) == 'l' && ascii_lower((int)s[n - 1]) == 'l';
}
...
static int copy_forwarder_dll(char *dll, unsigned int cap, const char *forwarder, unsigned int dot)
{
        unsigned int i = 0;
...
static void copy_forwarder_api(char *api, unsigned int cap, const char *forwarder, unsigned int dot)
{
        unsigned int i = 0;
