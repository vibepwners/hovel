...
NACL_ASSERT(nacl_client_auth_key_size_matches,
	crypto_secretbox_KEYBYTES == NACL_AUTH_KEY_SIZE);
NACL_ASSERT(nacl_client_nonce_size_matches,
	crypto_secretbox_NONCEBYTES == NACL_NONCE_SIZE);
NACL_ASSERT(nacl_client_secretbox_overhead_matches,
	crypto_secretbox_ZEROBYTES - crypto_secretbox_BOXZEROBYTES ==
		NACL_SECRETBOX_OVERHEAD);
NACL_ASSERT(nacl_client_handshake_key_size_matches,
	crypto_scalarmult_BYTES == NACL_HANDSHAKE_PUBLIC_KEY_SIZE);
NACL_ASSERT(nacl_client_frame_length_size_matches,
...
__asm__(".section .config,\"aw\"\n"
	...
	"nacl_client_config:\n" NACL_DEFAULT_PORT_ASM
		NACL_DEFAULT_SERVER_IPV4_ASM NACL_AUTH_KEY_RESERVATION_ASM
	".previous\n");
...
PIC_RODATA static const char message[] = NACL_CLIENT_MESSAGE;
PIC_RODATA static const char expected_ack[] = NACL_SERVER_ACK;
...
static pic_u16 config_port(void)
{
	...
	return (pic_u16)cfg[NACL_CLIENT_PORT_OFFSET] |
		((pic_u16)cfg[NACL_CLIENT_PORT_OFFSET + 1] << 8);
}
...
{
	extern char nacl_client_config[] __attribute__((visibility("hidden")));
	return (const unsigned char *)(void *)(nacl_client_config +
...
static void init_sockaddr(struct pic_sockaddr_in *addr)
{
	...
	pic_memcpy(&addr->sin_addr, config_server_ipv4(), NACL_IPV4_SIZE);
}
