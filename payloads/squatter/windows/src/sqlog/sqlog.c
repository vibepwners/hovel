...
static const char *const k_level_names[] = {
    "TRACE", "VERBOSE", "DEBUG", "INFO", "WARN", "ERROR", "FATAL", "OFF"
};
...
static BOOL CALLBACK init_cb(PINIT_ONCE once, PVOID param, PVOID *ctx)
{
    ...
    if (g.console != NULL && g.console != INVALID_HANDLE_VALUE &&
        GetConsoleMode(g.console, &mode) != FALSE &&
...
static void format_sys_error(unsigned long err, wchar_t *buf, int cap)
{
    ...
    written = FormatMessageW(
        ...
        (DWORD)err, MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), buf, (DWORD)cap,
        NULL);
    if (written == 0) {
        int pos = 0;
...
static void build_prefix(wchar_t *buf, int cap, int *pos, int sub, int level,
                         const char *file, int line, const char *func)
{
    ...
    append_fmt(buf, cap, pos,
               L"%.2u:%.2u:%.2u.%.3u %-7S [%-8S] tid=%-5lu %S:%d %S: ",
               (unsigned)now.wHour, (unsigned)now.wMinute,
               (unsigned)now.wSecond, (unsigned)now.wMilliseconds,
               sqlog_level_name(level), sqlog_subsystem_name((sqlog_subsystem)sub),
               (unsigned long)GetCurrentThreadId(), file, line, func);
}
...
static WORD level_color(int level)
{
    ...
    switch (level) {
    case SQLOG_LEVEL_TRACE:   return (WORD)FOREGROUND_INTENSITY;
    case SQLOG_LEVEL_VERBOSE: return (WORD)(FOREGROUND_BLUE | intense);
    case SQLOG_LEVEL_DEBUG:   return (WORD)(FOREGROUND_BLUE | FOREGROUND_GREEN | intense);
    case SQLOG_LEVEL_INFO:    return (WORD)(FOREGROUND_GREEN | intense);
    case SQLOG_LEVEL_WARN:    return (WORD)(FOREGROUND_RED | FOREGROUND_GREEN | intense);
    case SQLOG_LEVEL_ERROR:   return (WORD)(FOREGROUND_RED | intense);
    case SQLOG_LEVEL_FATAL:   return (WORD)(FOREGROUND_RED | FOREGROUND_BLUE | intense);
    default:                  return g.console_attrs;
    }
}
#endif
...
static void write_utf8(HANDLE h, const wchar_t *line, int wlen)
{
    ...
    n = WideCharToMultiByte(CP_UTF8, 0, line, wlen, utf8, (int)sizeof utf8,
                            NULL, NULL);
    if (n > 0) {
...
static void dispatch(int level, const wchar_t *line, int wlen)
{
    ...
    if (g.sinks[SQLOG_SINK_ID_DEBUGGER].enabled != 0 &&
        level >= (int)g.sinks[SQLOG_SINK_ID_DEBUGGER].level) {
        OutputDebugStringW(line);
    }
    if (g.sinks[SQLOG_SINK_ID_CONSOLE].enabled != 0 &&
        level >= (int)g.sinks[SQLOG_SINK_ID_CONSOLE].level) {
        write_console(level, line, wlen);
    }
    if (g.sinks[SQLOG_SINK_ID_FILE].enabled != 0 &&
        g.file != INVALID_HANDLE_VALUE &&
                ...
                const char *func, const wchar_t *fmt, ...)
{
    ...
    buf[0] = L'\0';
    build_prefix(buf, (int)(sizeof buf / sizeof buf[0]), &pos, sub, level, file,
                 line, func);
    va_start(ap, fmt);
...
void sqlog_hexdump(int sub, int level, const char *file, int line,
                   const char *func, const wchar_t *label, const void *data,
                   size_t len)
{
    ...
    ensure_init();
    sqlog_emit(sub, level, file, line, func, L"hexdump %s (%u bytes)",
               (label != NULL) ? label : L"", (unsigned)len);
    if (p == NULL) {
        return;
    }
    for (off = 0; off < len; off += 16) {
        ...
        for (i = 0; i < 16 && off + i < len; i++) {
            ...
            append_fmt(row, ROW_CAP, &pos, L"%c",
                       (c >= 0x20 && c < 0x7F) ? (wchar_t)c : L'.');
        }
        append_fmt(row, ROW_CAP, &pos, L"\r\n");
...
void sqlog__scope_enter(const sqlog_scope *s)
{
    if (sqlog_should(s->sub, SQLOG_LEVEL_TRACE)) {
        sqlog_emit(s->sub, SQLOG_LEVEL_TRACE, s->file, s->line, s->func,
                   L"-> enter %S", s->func);
    }
}
...
void sqlog_set_subsystem_level(sqlog_subsystem sub, int level)
{
    ...
    (void)InterlockedExchange(&g.sub_level[(int)sub],
                              (level < 0) ? -1 : (LONG)clamp_level(level));
}
...
int sqlog_open_file(const char *path)
{
    ...
    h = CreateFileW(wpath, FILE_APPEND_DATA, FILE_SHARE_READ, NULL, OPEN_ALWAYS,
