...
sq_status sq_net_startup(void)
{
        ...
        if (rc != 0)
        {
                ...
                SQLOG_WINERR(SQLOG_SUB_NET, ERROR, (unsigned long)rc, L"WSAStartup(2.2) failed");
                return SQ_ERR_SYSTEM;
        }
        if (LOBYTE(wsa.wVersion) != 2 || HIBYTE(wsa.wVersion) != 2)
        {
                SQLOG_ERROR(SQLOG_SUB_NET, L"Winsock 2.2 unavailable (got %u.%u)", (unsigned)LOBYTE(wsa.wVersion),
...
void sq_net_cleanup(void)
{
        if (WSACleanup() != 0)
        {
                SQLOG_WINERR(SQLOG_SUB_NET, ERROR, (unsigned long)WSAGetLastError(), L"WSACleanup failed");
        }
}
...
sq_status sq_net_tcp_socket(int family, SOCKET *out)
{
        ...
        *out = INVALID_SOCKET;
        ...
        s = WSASocketW(family, SOCK_STREAM, IPPROTO_TCP, NULL, 0, WSA_FLAG_OVERLAPPED);
        if (s == INVALID_SOCKET)
        {
                SQLOG_WINERR(SQLOG_SUB_NET, ERROR, (unsigned long)WSAGetLastError(), L"WSASocketW(family=%d) failed",
                             family);
                return SQ_ERR_SYSTEM;
        }
        *out = s;
...
void sq_net_close(SOCKET s)
{
        ...
        if (closesocket(s) != 0)
        {
                SQLOG_WINERR(SQLOG_SUB_NET, ERROR, (unsigned long)WSAGetLastError(), L"closesocket(%I64u) failed",
                             (ULONG64)s);
        }
}
...
static BOOL listen_candidate(ADDRINFOW *addr, int backlog, SOCKET *listener)
{
        ...
        if (setsockopt(*listener, SOL_SOCKET, SO_REUSEADDR, (const char *)&on, (int)sizeof on) != 0)
        {
        ...
        if (bind(*listener, addr->ai_addr, (int)addr->ai_addrlen) == 0 && listen(*listener, backlog) == 0)
        {
                return TRUE;
        }
        SQLOG_WINERR(SQLOG_SUB_NET, ERROR, (unsigned long)WSAGetLastError(), L"bind/listen on candidate failed");
        sq_net_close(*listener);
        *listener = INVALID_SOCKET;
        return FALSE;
}

static sq_status resolve_listen_addresses(const wchar_t *host, const wchar_t *port, ADDRINFOW **resolved)
{
        ADDRINFOW hints = {0};
        int gai = 0;
        ...
        SQLOG_WINERR(SQLOG_SUB_NET, ERROR, (unsigned long)gai, L"getaddrinfo(%s:%s) failed",
                     (host != NULL) ? host : L"*", port);
        return SQ_ERR_ADDRESS;
}

static BOOL choose_listener(ADDRINFOW *resolved, int backlog, SOCKET *out_listener, int *out_family)
{
        ADDRINFOW *cur = NULL;
...
sq_status sq_net_listen(const wchar_t *host, const wchar_t *port, int backlog, SOCKET *out_listener, int *out_family)
{
        ...
        *out_family = AF_UNSPEC;
        ...
        (void)choose_listener(resolved, effective_backlog, out_listener, out_family);
        FreeAddrInfoW(resolved);

        if (*out_listener == INVALID_SOCKET)
        {
                SQLOG_ERROR(SQLOG_SUB_NET, L"no usable listen address for %s:%s", (host != NULL) ? host : L"*", port);
                return SQ_ERR_ADDRESS;
        }
        return SQ_OK;
}
...
sq_status sq_net_load_ext(SOCKET s, sq_net_ext *out)
{
        ...
        if (WSAIoctl(s, SIO_GET_EXTENSION_FUNCTION_POINTER, &accept_guid, (DWORD)sizeof accept_guid, &out->accept_ex,
                     (DWORD)sizeof out->accept_ex, &got, NULL, NULL) != 0)
        {
                SQLOG_WINERR(SQLOG_SUB_NET, ERROR, (unsigned long)WSAGetLastError(), L"WSAIoctl(AcceptEx) failed");
                return SQ_ERR_SYSTEM;
        }
        if (WSAIoctl(s, SIO_GET_EXTENSION_FUNCTION_POINTER, &addrs_guid, (DWORD)sizeof addrs_guid,
                     &out->get_accept_ex_sockaddrs, (DWORD)sizeof out->get_accept_ex_sockaddrs, &got, NULL, NULL) != 0)
        {
                SQLOG_WINERR(SQLOG_SUB_NET, ERROR, (unsigned long)WSAGetLastError(),
...
void sq_net_peer_str(const struct sockaddr *addr, int addr_len, wchar_t *buf, size_t cap)
{
        ...
        rc = GetNameInfoW(addr, addr_len, host, (DWORD)(sizeof host / sizeof host[0]), serv,
                          (DWORD)(sizeof serv / sizeof serv[0]), NI_NUMERICHOST | NI_NUMERICSERV);
        if (rc != 0)
        {
