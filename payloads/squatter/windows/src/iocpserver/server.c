...
static sq_conn *conn_alloc(sq_server *s)
{
        ...
        if (c == NULL)
        {
                SQLOG_ERROR(SQLOG_SUB_SERVER, L"connection allocation failed (%u bytes)", (unsigned)sizeof *c);
                return NULL;
        }
        c->kind = SQ_OP_ACCEPT;
...
static sq_status post_accept(sq_server *s)
{
        ...
        ok = s->ext.accept_ex(s->listener, c->sock, c->accept_buf, 0, /* receive no data on accept */
                              (DWORD)SQ_ADDR_SLOT, (DWORD)SQ_ADDR_SLOT, &received, &c->overlapped);
        if (ok == FALSE)
        {
                int const err = WSAGetLastError();
                if (err != ERROR_IO_PENDING)
                {
                        SQLOG_WINERR(SQLOG_SUB_SERVER, ERROR, (unsigned long)err, L"AcceptEx failed");
                        conn_free(c);
...
static sq_status post_recv(sq_conn *c)
{
        ...
        if (rc != 0)
        {
                ...
                {
                        SQLOG_WINERR(SQLOG_SUB_SERVER, ERROR, (unsigned long)err, L"WSARecv failed");
                        return SQ_ERR_SYSTEM;
                }
        }
...
static sq_status post_send(sq_conn *c)
{
        ...
        if (rc != 0)
        {
                ...
                if (err != WSA_IO_PENDING)
                {
                        SQLOG_WINERR(SQLOG_SUB_SERVER, ERROR, (unsigned long)err, L"WSASend failed");
                        return SQ_ERR_SYSTEM;
                }
        }
        return SQ_OK;
...
static void log_accepted_peer(sq_server *s, sq_conn *c)
{
        ...
        s->ext.get_accept_ex_sockaddrs(c->accept_buf, 0, (DWORD)SQ_ADDR_SLOT, (DWORD)SQ_ADDR_SLOT, &local, &local_len,
                                       &remote, &remote_len);
...
static void on_accept_complete(sq_server *s, sq_conn *c, int failed)
{
        ...
        if (setsockopt(c->sock, SOL_SOCKET, SO_UPDATE_ACCEPT_CONTEXT, (const char *)&s->listener,
                       (int)sizeof s->listener) != 0)
        {
...
static void on_recv_complete(sq_server *s, sq_conn *c, DWORD bytes, int failed)
{
        ...
        produced = s->handler.on_recv(s->handler.user, c->rbuf, (size_t)bytes, c->sbuf, (size_t)SQ_CONN_BUF_LEN);
...
static void on_send_complete(sq_server *s, sq_conn *c, DWORD bytes, int failed)
{
        ...
        {
                if (post_send(c) != SQ_OK)
                { /* flush the remainder of a partial send */
                        conn_free(c);
                }
                return;
        }
...
static BOOL should_exit_worker(sq_server *s, const sq_iocp_event *ev, sq_status st)
{
...
static sq_status create_stop_event(sq_server *s)
{
        ...
        SQLOG_WINERR(SQLOG_SUB_SERVER, ERROR, (unsigned long)GetLastError(), L"CreateEvent failed");
        return SQ_ERR_SYSTEM;
}

static sq_status configure_counts(sq_server *s, const sq_server_config *cfg)
{
        s->worker_count = (cfg->worker_count > 0) ? cfg->worker_count : default_worker_count();
        if (s->worker_count > SQ_MAX_WORKERS)
        {
                s->worker_count = SQ_MAX_WORKERS;
        }
        s->accept_count = (cfg->accept_count > 0) ? cfg->accept_count : SQ_DEFAULT_ACCEPTS;
        s->workers = HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY, (SIZE_T)s->worker_count * sizeof *s->workers);
        return (s->workers != NULL) ? SQ_OK : SQ_ERR_NOMEM;
}

static sq_status open_listener_and_iocp(sq_server *s, const sq_server_config *cfg)
{
        sq_status st = sq_net_listen(cfg->host, cfg->port, cfg->backlog, &s->listener, &s->family);
...
static sq_status start_workers(sq_server *s)
{
        ...
        for (i = 0; i < s->worker_count; i++)
        {
                ...
                if (h == NULL)
                {
                        SQLOG_WINERR(SQLOG_SUB_SERVER, ERROR, (unsigned long)GetLastError(), L"CreateThread failed");
                        return SQ_ERR_SYSTEM;
                }
                s->workers[i] = h;
        }
...
sq_status sq_server_create(const sq_server_config *cfg, sq_server **out)
{
        ...
        *out = NULL;
        ...
        {
                SQLOG_ERROR(SQLOG_SUB_SERVER, L"sq_server_create: invalid configuration");
                return SQ_ERR_PARAM;
        }
        ...
        SQLOG_INFO(SQLOG_SUB_SERVER, L"listening on %s:%s (%u workers, %u accepts)",
                   (cfg->host != NULL) ? cfg->host : L"*", cfg->port, s->worker_count, s->accept_count);
        *out = s;
        return SQ_OK;
