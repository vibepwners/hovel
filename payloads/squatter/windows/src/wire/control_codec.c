...
static UINT32 open_encoded_size(const char *module, const char *const *args, int count)
{
...
BOOL sq_control_encode_open(const char *module, const char *const *args, int n_args, BYTE **out, UINT32 *out_len)
{
        int count = 0;
        UINT32 total = 0;
        BYTE *buf = NULL;
...
static BOOL decode_open_field(sqmux_OpenStream *out, UINT32 field, const BYTE *cursor, UINT32 value_len)
{
        if (field == 1u)
        {
                copy_bounded_len(out->module, cursor, value_len, (int)sizeof out->module);
                return TRUE;
        }
        if (field == 2u && out->args_count < SQMUX_OPEN_ARGS_MAX)
        {
                copy_bounded_len(out->args[out->args_count], cursor, value_len, (int)sizeof out->args[out->args_count]);
                out->args_count++;
        }
        return TRUE;
}
