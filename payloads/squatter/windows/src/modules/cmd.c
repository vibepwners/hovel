...
static BOOL write_utf8(HANDLE pipe, const wchar_t *message)
{
        ...
        n = WideCharToMultiByte(CP_UTF8, 0, message, -1, buf, (int)sizeof buf, NULL, NULL);
        if (n <= 1)
        {
                return FALSE;
        }
...
static BOOL interactive_requested(int argc, wchar_t **argv)
{
        ...
        if (argc == 2 && (arg_is_interactive(argv[1]) || arg_equal(argv[1], L"--debug")))
        {
                return TRUE;
        }
        if (argc == 3 && has_arg(argc, argv, L"--debug") && has_interactive_arg(argc, argv))
        {
                return TRUE;
        }
        return FALSE;
...
static void debug_line(HANDLE pipe, BOOL enabled, const wchar_t *event, DWORD value, DWORD err)
{
        wchar_t msg[SQ_CMD_MSG_MAX];

        if (enabled == FALSE || pipe == INVALID_HANDLE_VALUE || pipe == NULL || event == NULL)
        {
                return;
        }
        (void)wnsprintfW(msg, (int)(sizeof msg / sizeof msg[0]), L"[sqcmd] %s value=%lu err=%lu", event,
...
static BOOL write_exit(HANDLE pipe, DWORD exit_code)
{
        ...
        (void)wnsprintfW(msg, (int)(sizeof msg / sizeof msg[0]), L"exit=%lu", (unsigned long)exit_code);
        return write_utf8(pipe, msg);
}

static BOOL wait_overlapped(HANDLE h, OVERLAPPED *ov, DWORD *transferred, HANDLE debug_pipe, BOOL debug, HANDLE process)
{
        DWORD wait = 0;
        DWORD exit_code = 0;
        ...
        for (;;)
        {
                ...
                {
                        debug_line(debug_pipe, debug, L"read wait process exited", exit_code, 0);
                }
                else
                {
...
static BOOL finish_pending_read(HANDLE h, OVERLAPPED *ov, DWORD *out, HANDLE debug_pipe, BOOL debug, HANDLE process)
{
        BOOL ok = FALSE;
        DWORD exit_code = 0;

        debug_line(debug_pipe, debug, L"read pending", 0, ERROR_IO_PENDING);
        if (process != INVALID_HANDLE_VALUE && process != NULL && WaitForSingleObject(process, 0) == WAIT_OBJECT_0 &&
            GetExitCodeProcess(process, &exit_code) != FALSE)
        {
                debug_line(debug_pipe, debug, L"process exited while pending", exit_code, 0);
        }
        ok = wait_overlapped(h, ov, out, debug_pipe, debug, process);
        debug_line(debug_pipe, debug, ok ? L"read complete" : L"read wait failed", ok ? *out : 0,
                   ok ? 0 : GetLastError());
        return ok;
}

static BOOL read_file_overlapped(HANDLE h, BYTE *buf, DWORD cap, DWORD *out, HANDLE debug_pipe, BOOL debug,
                                 HANDLE process)
{
        OVERLAPPED ov;
        BOOL ok = FALSE;
...
static BOOL make_child_output_pipe(sq_cmd_pipe_pair *pair, HANDLE debug_pipe, BOOL debug)
{
        ...
        (void)wnsprintfW(name, (int)(sizeof name / sizeof name[0]), L"\\\\.\\pipe\\sqcmd-out-%lu-%lu",
                         (unsigned long)GetCurrentProcessId(), (unsigned long)seq);

        pair->read = CreateNamedPipeW(name, PIPE_ACCESS_INBOUND | FILE_FLAG_OVERLAPPED, PIPE_TYPE_BYTE | PIPE_WAIT, 1,
                                      SQ_CMD_IO_BUF, SQ_CMD_IO_BUF, 120 * 1000, NULL);
        if (pair->read == INVALID_HANDLE_VALUE)
        {
                debug_line(debug_pipe, debug, L"stdout read pipe failed", 0, GetLastError());
                return FALSE;
        }
        ...
        pair->write = CreateFileW(name, GENERIC_WRITE, 0, &sa, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
        if (pair->write == INVALID_HANDLE_VALUE)
        {
                debug_line(debug_pipe, debug, L"stdout write pipe failed", 0, GetLastError());
        ...
        (void)SetHandleInformation(pair->write, HANDLE_FLAG_INHERIT, HANDLE_FLAG_INHERIT);
        return TRUE;
}

static BOOL drain_child_output(HANDLE out_read, HANDLE module_pipe, BOOL overlapped, BOOL debug, HANDLE process)
{
        ...
        {
                if ((overlapped
                         ? read_file_overlapped(out_read, buf, (DWORD)sizeof buf, &got, module_pipe, debug, process)
                         : ReadFile(out_read, buf, (DWORD)sizeof buf, &got, NULL)) == FALSE)
                {
                        debug_line(module_pipe, debug, L"pump drain ended", 0, GetLastError());
                        return TRUE;
                }
                if (got == 0)
                {
                ...
                {
                        debug_line(module_pipe, debug, L"pump write failed", wrote, GetLastError());
                        return FALSE;
                }
        }
}
...
static DWORD WINAPI pump_child_output(LPVOID param)
{
        ...
        return drain_child_output(pump->out_read, pump->module_pipe, pump->overlapped, pump->debug, pump->process) ? 0
                                                                                                                   : 1;
}
...
static void stop_child_process(HANDLE process, HANDLE debug_pipe, BOOL debug)
{
        ...
        if (GetExitCodeProcess(process, &exit_code) != FALSE && exit_code == STILL_ACTIVE)
        {
...
static BOOL start_cmd_process(const wchar_t *command_line, HANDLE in_read, HANDLE out_write, PROCESS_INFORMATION *pi)
{
        STARTUPINFOW si;
        ...
        (void)append_wide(mutable_command, 0, (int)(sizeof mutable_command / sizeof mutable_command[0]), command_line);
...
static int run_one_shot(HANDLE output, int argc, wchar_t **argv)
{
        ...
        if (build_command_line(argc, argv, command_line, (int)(sizeof command_line / sizeof command_line[0])) <= 0)
        {
        ...
        if (start_cmd_process(command_line, GetStdHandle(STD_INPUT_HANDLE), out_write, &pi) == FALSE)
        {
        ...
        drained = drain_child_output(out_read, output, FALSE, FALSE, INVALID_HANDLE_VALUE);
...
static BOOL create_child_stdin(HANDLE *in_read, HANDLE *in_write, HANDLE output, BOOL debug)
{
        SECURITY_ATTRIBUTES sa;
        ...
        (void)SetHandleInformation(*in_read, HANDLE_FLAG_INHERIT, HANDLE_FLAG_INHERIT);
        (void)SetHandleInformation(*in_write, HANDLE_FLAG_INHERIT, 0);
        return TRUE;
}

static BOOL start_interactive_child(const wchar_t *command_line, HANDLE in_read, sq_cmd_pipe_pair *child_output,
                                    HANDLE output, BOOL debug, PROCESS_INFORMATION *pi)
{
        ...
        if (start_cmd_process(command_line, in_read, child_output->write, pi) == FALSE)
        {
...
static BOOL start_output_pump(sq_cmd_pipe_pair *child_output, PROCESS_INFORMATION *pi, HANDLE output, BOOL debug,
                              sq_cmd_pump *pump, HANDLE *pump_thread)
{
...
static void forward_operator_input(HANDLE input, HANDLE output, HANDLE in_write, HANDLE process, BOOL debug)
{
        BYTE buf[SQ_CMD_IO_BUF];
        DWORD got = 0;
        DWORD wrote = 0;
        ...
        for (;;)
        {
                ...
                if (WriteFile(in_write, buf, got, &wrote, NULL) == FALSE || wrote != got)
                {
                        debug_line(output, debug, L"stdin write failed", wrote, GetLastError());
                        (void)wrote;
                        return;
                }
                ...
                if (WaitForSingleObject(process, 0) == WAIT_OBJECT_0 &&
                    GetExitCodeProcess(process, &exit_code) != FALSE)
                {
                        debug_line(output, debug, L"process exited after input", exit_code, 0);
                }
        }
}
...
static int run_interactive(HANDLE input, HANDLE output, BOOL debug)
{
        ...
        if (build_interactive_command_line(command_line, (int)(sizeof command_line / sizeof command_line[0])) == FALSE)
        {
                (void)write_utf8(output, L"ERR command line too long");
                return 1;
        }
        ...
        if (!start_interactive_child(command_line, in_read, &child_output, output, debug, &pi))
        {
        ...
        if (!start_output_pump(&child_output, &pi, output, debug, &pump, &pump_thread))
        {
                (void)CloseHandle(in_write);
                close_pipe_pair(&child_output);
                (void)CloseHandle(pi.hThread);
