...
static BOOL write_file_chunk(HANDLE file, const BYTE *buf, DWORD got,
...
static BOOL send_putfile_ack(HANDLE output, unsigned long long total)
{
    ...
    (void)wnsprintfW(wack, (int)(sizeof wack / sizeof wack[0]), L"OK %I64u",
                     total);
    (void)WideCharToMultiByte(CP_UTF8, 0, wack, -1, ack, (int)sizeof ack, NULL,
                              NULL);
    return sq_xfer_send_stat(output, ack);
}

static int handle_putfile_message(HANDLE file, HANDLE output, const BYTE *buf,
                                  DWORD got, unsigned long long *total)
{
